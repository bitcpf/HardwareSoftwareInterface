...
int get_cache_size(int size) {
  ...
  while(1){
	  flush_cache();
	  for(i = 0; i <= count; i++){
		  access_cache(i*blk_size);
	//	  printf("Access mul block: %d %d \n", access_cache(i*blk_size),i);
